      ...
      <div className="overflow-y-auto flex-1 px-4 py-3 flex flex-col gap-2">
        {logs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-300 dark:text-slate-600">
            <Cpu size={36} />
            <p className="text-xs font-semibold uppercase tracking-[0.08em]">Nenhum log ainda</p>
          </div>
        ) : logs.map(entry => (
            ...
            {expandedId === entry.id && (
              <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-3 space-y-3 animate-in slide-in-from-top-2 duration-200">
                {entry.parsed_transaction && (
                  <div>
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-[0.08em] mb-1.5">Transação Parseada</p>
                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(entry.parsed_transaction).map(([k, v]) => v !== undefined && (
                        <div key={k} className="bg-white dark:bg-slate-800 rounded-xl px-3 py-2">
                          <p className="text-xs font-semibold text-slate-400 uppercase">{k}</p>
                          <p className="text-xs font-medium text-slate-800 dark:text-white truncate">{String(v)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {entry.error && (
                  <div className="p-3 bg-rose-50 dark:bg-rose-500/10 rounded-xl">
                    <p className="text-xs font-semibold text-rose-400 uppercase tracking-[0.08em] mb-1">Erro</p>
                    <p className="text-xs text-rose-600 font-medium">{entry.error}</p>
                  </div>
                )}
                {entry.raw_response && (
                  <div>
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-[0.08em] mb-1">Resposta Bruta</p>
                    <pre className="text-xs bg-slate-900 text-emerald-400 p-3 rounded-xl overflow-x-auto font-mono whitespace-pre-wrap">
                      {entry.raw_response.slice(0, 500)}{entry.raw_response.length > 500 ? '...' : ''}
                    </pre>
                  </div>
                )}
                <p className="text-xs text-slate-300 text-right">ID: {entry.id}</p>
              </div>
            )}
          </div>
        ))}
    ...
    <div className="flex flex-col h-full">
          ...
          {Object.entries(byType).map(([type, count]) => {
            ...
            return (
              <div key={type} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-xl ${meta.color} shrink-0`}>
                {meta.icon}
                <span className="text-xs font-semibold uppercase tracking-[0.08em]">{meta.label}</span>
      ...
      <div className="overflow-y-auto flex-1 px-4 py-3 flex flex-col gap-2">
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-300 dark:text-slate-600">
            <GitBranch size={36} />
            <p className="text-xs font-semibold uppercase tracking-[0.08em]">Nenhum evento ainda</p>
            <p className="text-xs text-slate-400">Adicione transações para gerar eventos</p>
          </div>
        ) : events.map(ev => {
          ...
          return (
            <div key={ev.id} className="bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden">
              <button className="w-full text-left px-4 py-3 flex items-center gap-3" onClick={() => setExpandedId(isExp ? null : ev.id)}>
                ...
                <div className="flex-1 min-w-0">
                  <span className={`text-xs font-semibold uppercase tracking-[0.08em] px-1.5 py-0.5 rounded-md ${meta.color}`}>{meta.label}</span>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="flex items-center gap-1 text-xs text-slate-400"><Clock size={8} />{formatTime(ev.created_at)}</span>
                    {description && <span className="text-xs text-slate-500 font-medium truncate max-w-[120px]">{description}</span>}
                    {count !== undefined && <span className="text-xs text-slate-400">{count} item{count !== 1 ? 's' : ''}</span>}
                  </div>
                </div>
              ...
              {isExp && (
                <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-3 animate-in slide-in-from-top-2 duration-200">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-[0.08em] mb-2">Payload</p>
                  <pre className="text-xs bg-slate-900 text-emerald-400 p-3 rounded-xl overflow-x-auto font-mono whitespace-pre-wrap">
                    {JSON.stringify(ev.payload, null, 2).slice(0, 600)}
                  </pre>
                  <p className="text-xs text-slate-300 text-right mt-2">ID: {ev.id}</p>
                </div>
              )}
            </div>
          );
            ...
            <div className="flex px-4 pt-3 gap-2 shrink-0 border-b border-slate-100 dark:border-slate-800">
              {([
                ...
                >
                  {tab.icon} {tab.label}
                  {tab.count > 0 && (
                    <span className="bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold rounded-full px-1.5 py-0.5 ml-0.5">{tab.count}</span>
                  )}
                </button>
              ))}
            </div>
