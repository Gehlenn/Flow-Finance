...
const Settings: React.FC<SettingsProps> = ({
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="bg-gradient-to-r from-indigo-600 to-violet-500 p-6 rounded-[2rem] flex justify-between items-center shadow-lg shadow-indigo-500/20 shrink-0 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-2xl font-black text-white tracking-tight leading-none">Operacao do workspace</h2>
          <p className="text-xs font-black text-white/70 uppercase tracking-widest mt-1.5">Perfil, acesso e faturamento</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-8">
        <div className="flex items-center gap-5 border-b border-slate-50 dark:border-slate-700 pb-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{userEmail}</p>
            <button onClick={() => setShowNameModal(true)} className="flex items-center gap-1.5 mt-2 text-xs font-black text-indigo-500 uppercase tracking-widest hover:text-indigo-600 transition-colors">
        ...
        <div className="space-y-4 border-t border-slate-50 dark:border-slate-700 pt-6">
          <div className="flex items-center justify-between px-1">
            <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Resumo do workspace</h4>
            <span className={`text-xs font-black uppercase tracking-widest ${currentPlan === 'pro' ? 'text-emerald-500' : 'text-indigo-500'}`}>
              {currentPlan.toUpperCase()} - {activeWorkspaceRole || activeWorkspace?.role || 'membro'}
            </span>
          </div>

          <div className="space-y-2">
            <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Workspace ativo</label>
            <select
          ...
          <div className="p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 space-y-2">
            {billingLoading ? (
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Carregando workspace...</p>
            ) : (
              <>
                <p className="text-sm font-black text-slate-800 dark:text-white">Plano atual: {planName}</p>
                <p className="text-xs font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">Tenant: {activeTenantName || activeWorkspace?.tenantName || 'Tenant ativo'}</p>
                <p className="text-xs font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">Workspace: {activeWorkspaceName || activeWorkspace?.name || 'Workspace ativo'}</p>
                <p className="text-xs font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">Papel: {activeWorkspaceRole || activeWorkspace?.role || 'membro'}</p>
                <p className="text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-widest">Mes atual: {monthlyUsageSummary}</p>
              </>
            )}
            {billingError && <p className="text-xs font-bold text-rose-500">{billingError}</p>}
            {billingDiagnostic && (
              <div role="status" className="rounded-2xl border border-rose-200 bg-rose-50 dark:bg-rose-500/10 p-3 space-y-1">
                <p className="text-xs font-black uppercase tracking-widest text-rose-700 dark:text-rose-300">{billingDiagnostic.title}</p>
                <p className="text-xs font-bold text-rose-700 dark:text-rose-100">{billingDiagnostic.message}</p>
                <p className="text-xs font-black uppercase tracking-widest text-rose-600 dark:text-rose-300">Próximo passo: {billingDiagnostic.suggestion}</p>
              </div>
            )}
          </div>
          ...
          {canOpenWorkspaceAdmin && (
            <div className="p-4 rounded-2xl border border-dashed border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-500/5 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-300">Admin do workspace</p>
              ...
              <button
                ...
                className="w-full p-4 rounded-2xl bg-indigo-600 text-white text-xs font-black uppercase tracking-widest disabled:opacity-50"
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-3">
              ...
              <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Acesso social</h4>
            </div>
            {linkFeedback && (
              linkFeedback.type === 'success' ? (
                <div className="flex items-center gap-1.5 animate-in slide-in-from-right-2 text-emerald-500">
                  <CheckCircle2 size={12} />
                  <span className="text-xs font-black uppercase tracking-widest">{linkFeedback.msg}</span>
                </div>
              ) : (
                <div role="status" className="rounded-2xl border border-rose-100 bg-rose-50/80 p-3 animate-in slide-in-from-right-2 dark:border-rose-500/20 dark:bg-rose-500/10">
                  <div className="flex items-start gap-2">
                    <AlertCircle size={12} className="mt-0.5 text-rose-500" />
                    <div className="min-w-0">
                      <p className="text-xs font-black uppercase tracking-widest text-rose-600">{linkFeedback.diagnostic?.title ?? 'Falha ao vincular provedor'}</p>
                      <p className="mt-1 text-xs font-bold text-slate-600 dark:text-slate-300">{linkFeedback.diagnostic?.message ?? linkFeedback.msg}</p>
                      <p className="mt-1 text-xs font-bold text-rose-500">{linkFeedback.diagnostic?.suggestion ?? 'Tente novamente em alguns instantes.'}</p>
                    </div>
                  </div>
                </div>
              )
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <span className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Vincular Google</span>
            </button>
            <button
            ...
            >
              ...
              <span className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Vincular Apple</span>
            </button>
          </div>
          <p className="text-xs text-center font-bold text-slate-400 uppercase tracking-widest px-2">Vincule um provedor para entrar mais rapido na proxima sessao.</p>
        </div>

        <div className="space-y-4">
          <div className="flex items-center gap-3 px-1">
            <Moon size={16} className="text-indigo-600 dark:text-indigo-400" />
            <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Tema</h4>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => handleThemeChange('dark')} className={`flex-1 py-5 rounded-3xl flex flex-col items-center gap-2 border-2 transition-all ${theme === 'dark' ? 'bg-indigo-500/10 border-indigo-500 text-indigo-400 shadow-xl shadow-indigo-900/10' : 'bg-slate-50 dark:bg-slate-900 border-transparent text-slate-400 hover:border-slate-200'}`}>
              ...
              <span className="text-xs font-black uppercase tracking-widest">Escuro</span>
            </button>
          </div>
        </div>

        <button onClick={onLogout} className="w-full py-5 bg-rose-50 dark:bg-rose-500/10 text-rose-500 rounded-3xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-rose-100 dark:hover:bg-rose-500/20 transition-all border border-rose-100 dark:border-rose-500/20">
          <LogOut size={18} /> Sair
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-4">
        <div className="flex items-center gap-3 border-b border-slate-50 dark:border-slate-700 pb-2">
          ...
          <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">Integracoes</h4>
        </div>
        ...
        <div className="p-5 bg-slate-50 dark:bg-slate-900 rounded-2xl space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h5 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-tight">Chave de integração</h5>
              <p className="text-xs text-slate-400 font-medium mt-0.5">Use para enviar eventos operacionais via webhook</p>
              ...
              {integrationKeyMetaError && (
                <div className="mt-2 space-y-2">
                  <p className="text-xs text-amber-600 dark:text-amber-300 font-bold">{integrationKeyMetaError}</p>
                  {integrationKeyMetaDiagnostic && (
                    <div role="status" className="rounded-2xl border border-amber-200 bg-amber-50 dark:bg-amber-500/10 p-3 space-y-1">
                      <p className="text-xs font-black uppercase tracking-widest text-amber-700 dark:text-amber-300">{integrationKeyMetaDiagnostic.title}</p>
                      <p className="text-xs font-bold text-amber-800 dark:text-amber-100">{integrationKeyMetaDiagnostic.message}</p>
                      <p className="text-xs font-black uppercase tracking-widest text-amber-600 dark:text-amber-300">Proximo passo: {integrationKeyMetaDiagnostic.suggestion}</p>
                    </div>
                  )}
                </div>
              )}
          ...
          {integrationKeyError && (
            <div className="space-y-2">
              <p className="text-xs text-rose-500 font-bold">{integrationKeyError}</p>
              {integrationKeyDiagnostic && (
                <div role="status" className="rounded-2xl border border-rose-200 bg-rose-50 dark:bg-rose-500/10 p-3 space-y-1">
                  <p className="text-xs font-black uppercase tracking-widest text-rose-700 dark:text-rose-300">{integrationKeyDiagnostic.title}</p>
                  <p className="text-xs font-bold text-rose-700 dark:text-rose-100">{integrationKeyDiagnostic.message}</p>
                  <p className="text-xs font-black uppercase tracking-widest text-rose-600 dark:text-rose-300">Próximo passo: {integrationKeyDiagnostic.suggestion}</p>
                </div>
              )}
            </div>
          )}
          ...
          {clipboardDiagnostic && (
            <div role="status" className="rounded-2xl border border-amber-200 bg-amber-50 dark:bg-amber-500/10 p-3 space-y-1">
              <p className="text-xs font-black uppercase tracking-widest text-amber-700 dark:text-amber-300">{clipboardDiagnostic.title}</p>
              <p className="text-xs font-bold text-amber-800 dark:text-amber-100">{clipboardDiagnostic.message}</p>
              <p className="text-xs font-black uppercase tracking-widest text-amber-600 dark:text-amber-300">Proximo passo: {clipboardDiagnostic.suggestion}</p>
            </div>
          )}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-700 transition-colors disabled:opacity-50"
            >
            ...
            {integrationKeyConfigured && (
              <button
                ...
                disabled={integrationKeyLoading}
                className="flex items-center gap-1.5 px-3 py-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-rose-100 transition-colors disabled:opacity-50"
              >
                <Trash2 size={11} />
                Revogar chave
              </button>
          ...
          <details className="group">
            <summary className="text-xs font-black uppercase tracking-widest text-slate-400 cursor-pointer hover:text-indigo-500 transition-colors list-none flex items-center gap-1">
              <ChevronRight size={10} className="group-open:rotate-90 transition-transform" />
              Como usar — guia rapido
            </summary>
            <div className="mt-3 space-y-3">
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-xs font-black text-indigo-500 uppercase tracking-widest">1 — Gere a chave acima</p>
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-xs font-black text-indigo-500 uppercase tracking-widest">2 — No n8n: nó HTTP Request</p>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">Header: <code className="bg-slate-200 dark:bg-slate-700 px-1 rounded">X-Integration-Key: flw_sua_chave</code></p>
              </div>
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-xs font-black text-indigo-500 uppercase tracking-widest">3 — Categorias aceitas</p>
                <div className="flex flex-wrap gap-1 mt-1">
                  {(['Pessoal', 'Trabalho / Consultório', 'Negócio', 'Investimento'] as const).map(cat => (
                    <span key={cat} className="text-xs font-mono bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded">{cat}</span>
                  ))}
                </div>
              </div>
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-xs font-black text-indigo-500 uppercase tracking-widest">4 — Tipos de evento</p>
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-black text-indigo-500 uppercase tracking-widest">Payload minimo (payment_received)</p>
                  <button
                    onClick={handleCopyPayload}
                    aria-label="Copiar payload de integracao"
                    className="flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-200 transition-colors text-xs font-bold"
                  >
  ...
  "payload": {
    ...
    "description": "Consulta - Maria"
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-black text-indigo-500 uppercase tracking-widest">Testar via curl</p>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-300 transition-colors text-xs font-bold"
                  >
  ...
  -H "X-Integration-Key: flw_sua_chave"`}</pre>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-4">
        <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest border-b border-slate-50 dark:border-slate-700 pb-2">Suporte operacional</h4>
        <div className="grid grid-cols-1 gap-3">
          ...
          >
            ...
            <div className="flex-1">
              <h5 className="text-xs font-black text-slate-800 dark:text-white uppercase">Guia com IA</h5>
          ...
          >
            ...
            <div className="flex-1">
              <h5 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-tight">Suporte humano</h5>
          ...
          >
            ...
            <div className="flex-1">
              <h5 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-tight">Termos e privacidade</h5>
      ...
      <div className="flex justify-center pt-8 pb-4">
        ...
        >
          <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 flex items-center gap-2">
            ...
            <span className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest group-hover:text-indigo-500 transition-colors">Flow Finance 2026</span>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 w-full max-w-lg max-h-[85vh] rounded-[3rem] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">Guia com IA</h3>
              </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              {supportDiagnostic && (
                <div className="rounded-[2rem] border border-amber-200 bg-amber-50 p-5 text-amber-950 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
                  <p className="text-xs font-black uppercase tracking-widest">Diagnóstico do guia com IA</p>
                  <p className="mt-2 text-sm font-bold leading-relaxed">{supportDiagnostic.message}</p>
              ...
              {isGeneratingSupport && (
                <div className="py-20 flex flex-col items-center gap-4 text-center">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest text-indigo-600 animate-pulse">Processando...</p>
                </div>
              )}
              {supportResponse && (
