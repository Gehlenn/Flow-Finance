...
const Settings: React.FC<SettingsProps> = ({
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="bg-gradient-to-r from-indigo-600 to-violet-500 p-6 rounded-[2rem] flex justify-between items-center shadow-lg shadow-indigo-500/20 shrink-0 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-2xl font-semibold text-white tracking-tight leading-none">Operacao do workspace</h2>
          <p className="text-sm font-semibold text-white/70 uppercase tracking-[0.16em] mt-1.5">Perfil, acesso e faturamento</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-8">
        <div className="flex items-center gap-5 border-b border-slate-50 dark:border-slate-700 pb-8">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-slate-800 dark:text-white text-xl tracking-tight leading-none mb-1">{userName || 'Usuario Flow'}</h3>
            <p className="text-sm font-medium text-slate-400 uppercase tracking-[0.16em]">{userEmail}</p>
            <button onClick={() => setShowNameModal(true)} className="flex items-center gap-1.5 mt-2 text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em] hover:text-indigo-600 transition-colors">
        ...
        <div className="space-y-4 border-t border-slate-50 dark:border-slate-700 pt-6">
          <div className="flex items-center justify-between px-1">
            ...
            <span className={`text-sm font-semibold uppercase tracking-[0.16em] ${currentPlan === 'pro' ? 'text-emerald-500' : 'text-indigo-500'}`}>
          ...
          {canOpenWorkspaceAdmin && (
            <div className="p-4 rounded-2xl border border-dashed border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-500/5 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.16em] text-indigo-600 dark:text-indigo-300">Admin do workspace</p>
              ...
              <button
                ...
                className="w-full p-4 rounded-2xl bg-indigo-600 text-white text-sm font-semibold uppercase tracking-[0.16em] disabled:opacity-50"
              >
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-center font-medium text-slate-400 uppercase tracking-[0.16em] px-2">Vincule um provedor para entrar mais rapido na proxima sessao.</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={() => handleThemeChange('light')} className={`flex-1 py-5 rounded-3xl flex flex-col items-center gap-2 border-2 transition-all ${theme === 'light' ? 'bg-indigo-50/50 dark:bg-indigo-500/10 border-indigo-500 text-indigo-600 dark:text-indigo-400 shadow-lg' : 'bg-slate-50 dark:bg-slate-900 border-transparent text-slate-400 hover:border-slate-200'}`}>
              ...
              <span className="text-sm font-semibold uppercase tracking-[0.16em]">Claro</span>
            </button>
            <button onClick={() => handleThemeChange('dark')} className={`flex-1 py-5 rounded-3xl flex flex-col items-center gap-2 border-2 transition-all ${theme === 'dark' ? 'bg-indigo-500/10 border-indigo-500 text-indigo-400 shadow-xl shadow-indigo-900/10' : 'bg-slate-50 dark:bg-slate-900 border-transparent text-slate-400 hover:border-slate-200'}`}>
              ...
              <span className="text-sm font-semibold uppercase tracking-[0.16em]">Escuro</span>
            </button>
          </div>
        </div>

        <button onClick={onLogout} className="w-full py-5 bg-rose-50 dark:bg-rose-500/10 text-rose-500 rounded-3xl font-semibold text-sm uppercase tracking-[0.16em] flex items-center justify-center gap-3 hover:bg-rose-100 dark:hover:bg-rose-500/20 transition-all border border-rose-100 dark:border-rose-500/20">
          <LogOut size={18} /> Sair
        </button>
      </div>
        ...
        <div className="p-5 bg-slate-50 dark:bg-slate-900 rounded-2xl space-y-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-xl text-sm font-semibold uppercase tracking-[0.16em] hover:bg-indigo-700 transition-colors disabled:opacity-50"
            >
            ...
            {integrationKeyConfigured && (
              <button
                ...
                disabled={integrationKeyLoading}
                className="flex items-center gap-1.5 px-3 py-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl text-sm font-semibold uppercase tracking-[0.16em] hover:bg-rose-100 transition-colors disabled:opacity-50"
              >
                <Trash2 size={11} />
                Revogar chave
              </button>
          ...
          <details className="group">
            <summary className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-400 cursor-pointer hover:text-indigo-500 transition-colors list-none flex items-center gap-1">
              <ChevronRight size={10} className="group-open:rotate-90 transition-transform" />
              Como usar — guia rapido
            </summary>
            <div className="mt-3 space-y-3">
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em]">1 — Gere a chave acima</p>
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em]">2 — No n8n: nó HTTP Request</p>
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em]">3 — Categorias aceitas</p>
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <p className="text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em]">4 — Tipos de evento</p>
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em]">Payload minimo (payment_received)</p>
                  <button
                    onClick={handleCopyPayload}
                    aria-label="Copiar payload de integracao"
                    className="flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-200 transition-colors text-sm font-semibold"
                  >
  ...
  "payload": {
    ...
    "description": "Consulta - Maria"
              ...
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-indigo-500 uppercase tracking-[0.16em]">Testar via curl</p>
  ...
  -H "X-Integration-Key: flw_sua_chave"`}</pre>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-700 shadow-sm space-y-4">
        <h4 className="text-sm font-semibold text-slate-800 dark:text-white uppercase tracking-[0.16em] border-b border-slate-50 dark:border-slate-700 pb-2">Suporte operacional</h4>
        <div className="grid grid-cols-1 gap-3">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-400 font-medium">Tire dúvidas sobre caixa, integrações ou fluxo do produto</p>
            </div>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-400 font-medium">Fale com a equipe pelo WhatsApp</p>
            </div>
            <ChevronRight size={16} className="text-slate-300" />
          </a>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-400 font-medium">Consulte as politicas de uso e protecao de dados</p>
            </div>
            <ChevronRight size={16} className="text-slate-300" />
          </button>
        </div>
      ...
      <div className="flex justify-center pt-8 pb-4">
        ...
        >
          <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-[0.16em] group-hover:text-indigo-500 transition-colors">Flow Finance 2026</span>
          </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
                ...
                <div className="text-center py-10 space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2 px-4">
                    {['Ajuda com fluxo de caixa', 'Como devo usar meu saldo agora?', 'Como exportar meus dados?'].map((question) => (
                      <button key={question} onClick={() => { setSupportQuery(question); void handleAiSupport(); }} className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-indigo-50 transition-all text-left border border-transparent hover:border-indigo-100">{question}</button>
                    ))}
                  </div>
                </div>
              )}
              {isGeneratingSupport && (
                <div className="py-20 flex flex-col items-center gap-4 text-center">
                  ...
                  <p className="text-sm font-semibold uppercase tracking-[0.16em] text-indigo-600 animate-pulse">Processando...</p>
                </div>
              )}
              {supportResponse && (
