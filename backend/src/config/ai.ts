...
export async function generateContent(
  ...
  if (hasOpenAI) {
    try {
      logger.debug('Attempting OpenAI request');
      ...
      if (hasGemini && (error.status === 429 || error.status === 404)) {
        logger.warn({ error: error.message, status: error.status }, 'OpenAI failed, falling back to Gemini');
  ...
  if (hasGemini) {
    logger.debug('Using Gemini as primary AI provider');
    return await gemini.generateContent(prompt, options);
  }
