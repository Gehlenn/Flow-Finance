...
router.post('/push', validate(SyncPushSchema), asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await pushSyncItems(userId, payload.entity, payload.items);
...
router.get('/pull', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await pullSyncItems(userId, since);
  res.json(result);
}));
