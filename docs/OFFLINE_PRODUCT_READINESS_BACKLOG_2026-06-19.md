...
- Documented: the manifest runner records paths, hashes, console issues, and page errors; pixel comparison is handled by the separate Playwright gate below.
...
- Implemented: `tests/e2e/visual-diff.spec.ts` and its 14 committed Chromium/Windows snapshots now add an actual pixel-diff gate for `dashboard`, `flow`, `analytics`, `cfo`, `assistant`, `goals`, and `insights` across desktop `1440x900` and mobile `390x844`; `npm run health:visual-diff` blocks when more than `0.2%` of pixels drift beyond Playwright's `0.2` color threshold.
...
- Inferred: the visual baseline is now materially stronger for offline review, but it is still deterministic screenshot evidence, not user preference or conversion evidence.
...
- P2/P3 UI/product debt remains: legal modal density, possible generic wording on secondary surfaces, and a future Linux/CI visual baseline if this local Chromium/Windows gate becomes a release blocker.
...
3. Optional P2/P3 product decision for secondary production surfaces: integrations/n8n, workspace administration, revenue history, goals, and legal-modal density; do not cut required SaaS controls without an explicit scope decision.
4. Resume real-use evidence collection for habit/cohort state when app testing becomes available.

Latest evidence refresh on 2026-07-12: activation refresh, AI quality, claims, and core visual regression are `PASS`; `test-results/audit-evidence/2026-07-12T19-10-47-925Z/report.json` remains `BLOCK` only because habit proof has not reached `2` review weeks / `7` observation days and the workspace remains in `revisao_1_semana`.
