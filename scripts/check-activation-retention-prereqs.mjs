...
function evaluateActivationRetentionReadiness(env) {
...
function printHumanReport({ envFilePath, result }) {
  process.stdout.write('Flow Finance - Activation/Retention Prereqs\n');
  ...
  if (result.ready) {
    ...
    process.stdout.write(`READY: activation/retention export prerequisites are satisfied via ${mode}.\n`);
    return;
  }
...
async function run() {
  ...
  const result = evaluateActivationRetentionReadiness(mergedEnv);
